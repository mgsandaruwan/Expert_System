...
public class expertsystem2 {
    ...
    private static String[] allSymptoms = {"unexplained_weight_loss", "fatigue", "persistent_pain", "changes_in_bowel_bladder_habits", 
        "persistent_cough_or_hoarseness", "changes_in_moles_or_skin_lesions", "unexplained_bleeding_or_bruising", "difficulty_swallowing_or_indigestion",
        "lumps_or_swelling"};
    ...
    public static void main(String[] args) {
        ...
        javax.swing.SwingUtilities.invokeLater(expertsystem2::createAndShowGUI2);
    }
    ...
    public static void createAndShowGUI2() {
        ...
        frame.add(panel);
        layoutComponents1(panel);

        frame.setVisible(true);
    }


    private static void layoutComponents1(JPanel panel) {
    ...
    "Symptoms for these diseases (unexplained_weight_loss, fatigue, persistent_pain, changes_in_bowel_bladder_habits, persistent_cough_or_hoarseness, changes_in_moles_or_skin_lesions, unexplained_bleeding_or_bruising, heart difficulty_swallowing_or_indigestion,lumps_or_swelling) have been presented here.</font></center></html>");
        ...
        startButton.addActionListener(e -> {
            ...
            showNextSymptom1(panel, nameField.getText(), nameField);
        }
        ...
        private static void showNextSymptom1(JPanel panel, String patientName, JTextField nameField) {
        if (currentSymptomIndex >= allSymptoms.length) {
            // Done gathering symptoms, show diagnosis
            displayDiagnosis1(panel, patientName);
            return;
        }
        ...
        yesButton.addActionListener(e -> {
            ...
            panel.removeAll();
            currentSymptomIndex++;
            showNextSymptom1(panel, patientName, nameField); // Show next symptom
        });
    ...
    private static void displayDiagnosis1(JPanel panel, String patientName) {
        String query = "diagnose_cancer([" + String.join(",", patientSymptoms) + "], CancerTypes).";
        ...
        for (java.util.Map<String, Term> solution : solutions) {
            Term diseaseTerm = solution.get("CancerTypes");
            String disease = diseaseTerm.toString();
            CancerTypes.add(disease);
        }

        StringBuilder diseasesText = new StringBuilder();
        for (String disease : CancerTypes) {
